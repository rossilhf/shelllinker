...
class Log:
    ...
    def __init__(self, myfilename):
        self.logName = os.path.join(log_path, '%s.log' % (myfilename+time.strftime('%Y-%m-%d')))  # 文件的命名
        self.logger = logging.getLogger()
        #self.logger.setLevel(logging.DEBUG)
        self.logger.setLevel(logging.INFO)
        self.formatter = colorlog.ColoredFormatter('%(log_color)s[%(asctime)s] [%(filename)s:%(lineno)d] [%(module)s:%(funcName)s] [%(levelname)s]- %(message)s', log_colors=log_colors_config)  # 日志输出格式
        self.handle_logs()
            ...
            if file_list:  # 目录下没有日志文件
                for i in file_list:
                    ...
                    if (now - t).days > 6:  # 创建时间大于6天的文件删除
                        print("self.delete_logs(file_path)...")
