...
if __name__ == '__main__':
    ...
    # 声明/指定消息队列
    exchange_name = 'rossi_exchange_direct'
    queue_name = "myqueue2_"+str(time.time())
    topic_key = "qdh_mytest"

    channel.exchange_declare(exchange=exchange_name, exchange_type='direct')
    channel.queue_declare(queue=queue_name, auto_delete=True)#, durable=True)
    
    channel.queue_bind(exchange='rossi_exchange_direct', queue=queue_name, routing_key=topic_key)
